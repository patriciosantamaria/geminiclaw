---
name: vopak-wizard-toolkit
description: Advanced technical patterns from the Google Gemini Cookbook. Includes logic for 3D Spatial Reasoning, Batch API processing, and Multi-Agent Orchestration.
---

# Vopak Wizard's Toolkit Skill

This skill provides the high-end technical "spells" for the Ultimate Vopak AI Assistant.

## 🏗️ Advanced Patterns
### 1. 3D Spatial Reasoning (Tank Inspection)
- Use **Spatial Understanding** prompts to locate defects relative to tank geometry.
- Correlate drone XY coordinates with structural blueprints.

### 2. The Economical Archivist (Batch Processing)
- Use the **Vertex AI Batch API** for high-volume PDF/Log processing.
- Implementation: `gcloud ai batch-prediction-jobs create` for cost-optimized historical audits.

### 3. Multi-Agent Orchestration (The Council)
- Use **LangGraph-inspired** state management to coordinate between sub-agents (e.g., vopak-transformation-lead and vopak-solution-architect).
- Each sub-agent maintains its own 'memory' but reports to the 'High Wizard' (this assistant).

## 🛡️ Mandates
- **Grounding First:** Always ground technical advice in Vopak's internal technical documentation or official GCP references.
- **ROI Tracking:** Every batch job must report the 'Estimated Cost Savings' compared to standard API calls.
