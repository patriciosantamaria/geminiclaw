...
2.  **Embedded Knowledge Engine (SQLite FTS5):** Stores unstructured context, previous meeting summaries, technical research, and **Google Chat Context**.
...
The `vopak-inbox-triage` subagent now assigns a complexity score (1-5) to incoming communications. For scores >3, the agent autonomously proposes "Deep Work" sessions in the user's calendar.
...
1.  **`read_workspace_script` (Safe):** Used for discovering context, scanning the Inbox for "real humans" via the `vopak-inbox-triage` subagent, and fetching Contacts/Tasks.
